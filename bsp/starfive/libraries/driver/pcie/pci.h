...
const char * pci_class_str(uint8_t class);
 ...
 * */
uint32_t pci_read_bar32(struct pci_controller *hose, pci_dev_t dev, int barnum);
...
struct dm_pci_ops {
	...
	int (*read_config)(const struct udevice *bus, pci_dev_t bdf,
			   unsigned int offset, unsigned long *valuep, enum pci_size_t size);
	/**
...
#define pci_get_ops(dev)	((struct dm_pci_ops *)(dev)->ops)
 ...
 */
int dm_pci_clrset_config8(struct udevice *dev, int offset, uint32_t clr, uint32_t set);
int dm_pci_clrset_config16(struct udevice *dev, int offset, uint32_t clr, uint32_t set);
int dm_pci_clrset_config32(struct udevice *dev, int offset, uint32_t clr, uint32_t set);
 ...
 */
int pci_write_config32(pci_dev_t pcidev, int offset, uint32_t value);
int pci_write_config16(pci_dev_t pcidev, int offset, uint16_t value);
int pci_write_config8(pci_dev_t pcidev, int offset, uint8_t value);
int pci_read_config32(pci_dev_t pcidev, int offset, uint32_t *valuep);
int pci_read_config16(pci_dev_t pcidev, int offset, uint16_t *valuep);
int pci_read_config8(pci_dev_t pcidev, int offset, uint8_t *valuep);
 ...
 */
 #endif
int dm_pci_find_next_capability(struct udevice *dev, uint8_t start, int cap);
 ...
 * @return 0 if found, -ve on error
 */
int dm_pci_find_class(unsigned int find_class, int index, struct udevice **devp);
...
struct dm_pci_emul_ops {
	 ...
	 * @return 0 if OK, -ve on error
	 */
	int (*read_config)(const struct udevice *dev, unsigned int offset,
			   unsigned long *valuep, enum pci_size_t size);
	/**
	 * write_config() - Write a PCI configuration value
	 *
...
static inline struct pci_child_plat *dev_get_parent_plat(struct udevice *udev)
{
	return &udev->pplat;
}

static inline pci_dev_t dm_pci_get_bdf(struct udevice *dev)
{
...
static inline struct pci_controller *pci_get_controller(struct udevice *dev)
{
	return dev->hose;
}
