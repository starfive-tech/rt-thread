	...
	for (bar = PCI_BASE_ADDRESS_0;
		...
		if (bar_response & PCI_BASE_ADDRESS_SPACE) {
			...
			if ((bar_response & PCI_BASE_ADDRESS_MEM_TYPE_MASK) ==
				...
				bar64 = ((uint64_t)bar_response_upper << 32) |
						bar_response;
				...
				bar_size = (uint32_t)(~(bar_response &
		...
		if (!ret) {
			...
			dm_pci_write_config32(dev, bar, (uint32_t)bar_value);

			if (found_mem64) {
				bar += 4;
#ifdef CONFIG_SYS_PCI_64BIT
				dm_pci_write_config32(dev, bar,
						      (uint32_t)(bar_value >> 32));
#else
				/*
...
void dm_pciauto_prescan_setup_bridge(struct udevice *dev, int sub_bus)
{
	...
	uint16_t cmdstat, prefechable_64;
	//struct udevice *ctlr = pci_get_controller(dev);
	struct pci_controller *ctlr_hose = pci_get_controller(dev);
	...
	dm_pci_write_config8(dev, PCI_PRIMARY_BUS,
			     PCI_BUS(dm_pci_get_bdf(dev)) - dev_seq(dev));
	dm_pci_write_config8(dev, PCI_SECONDARY_BUS, sub_bus - dev_seq(dev));
...
void dm_pciauto_postscan_setup_bridge(struct udevice *dev, int sub_bus)
{
	...
	/* Configure bus number registers */
	dm_pci_write_config8(dev, PCI_SUBORDINATE_BUS, sub_bus - dev_seq(dev));
